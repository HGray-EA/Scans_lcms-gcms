...
---
## Introduction
"Scans" data was pulled from CANARY, this includes LCMS and GCMS data, sites from the RSN network with "scans" data were selected for analysis.
...
The below plot shows the seasonal variation in the log of concentration for all compounds through the seasons- summer and autumn have a median concentration higher than winter and spring. This shows there is a temporal variation in concentration of all compounds when grouped together. 
...
# Visualise   
 ...
 plt <-  ggplot(RSN_MS, aes(x = season, y =logConc, fill=season))+
...
selectInput("Comp", "Select Compound", choices= unique(RSN_MS$Compound_Name))
```


```{r, Temporal trend of a given compound}
#| context: server

filt_dat = reactive({
                  RSN_MS[RSN_MS$Compound_Name == input$Comp,] 
                    })
 
output$plt <- renderPlot({
              
ggplot(filt_dat, aes(x = as.factor(season), y = logConc, fill=season))+
 geom_violin() + geom_boxplot(width=0.3)+labs(title = paste0("Concentraion by Season", input$Comp))
...
Below we use a random effect mixed model, whereby we let each site have a random intercept `r (1|Sample_Site_ID)`, so each site can start at a given concentration but the trend for that site is the same across all sites (fixed). 
...
#| code-fold: true
 ...
 lme1 <- lme4::lmer(logConc ~ (1| Sample_Site_ID) + (1| season), data=RSN_MS)
  summary(lme1)
...
#| code-fold: true
 lme2 <- lme4::lmer(logConc ~ (1| Sample_Site_ID) + (1 | month), data = RSN_MS)
 summary(lme2)
