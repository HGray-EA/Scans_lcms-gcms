...
# Transform 
 # names(LCMS_Open_Tgt)==names(GCMS_Open_Tgt) check colnames are the same before rbind
  Open_Lib <- rbind(EA_NLS_Target_LCMS, EA_NLS_Target_GCMS)
...
pal1 <- MetBrewer::met.brewer("Monet",612)

RSN_MS %>% 
  count(Compound_Name) %>% 
  rename(freq = n) %>% 
  arrange(desc(freq)) %>% 
  wordcloud2(., size = 1.5, color = pal, backgroundColor = "white")
...
pal2 <- MetBrewer::met.brewer("Monet",25)
...
RSN_k2 <- RSN_k %>%
  ...
  select(norm_conc, Sample_Site_ID, SMC_DESC, season) %>%
  mutate(
    norm_conc = as.numeric(norm_conc),   # convert from 1-column matrix to numeric
    Sample_Site_ID = as.factor(Sample_Site_ID),
    SMC_DESC = as.factor(SMC_DESC),
    season = as.factor(season)
  )

# compute Gower distance
gower_dist <- cluster::daisy(RSN_k2, metric = "gower")


# Classical MDS to convert distance matrix to numeric coordinates
mds_coords <- cmdscale(gower_dist, k = 5)  # k dimensions (can tune)

# SRun k-means on MDS coordinates
set.seed(123)
k <- 10
km <- kmeans(mds_coords, centers = k)

# Attach cluster info
RSN_k$cluster <- km$cluster

# Pick representative compound per cluster
representatives <- RSN_k %>%
  mutate(row_id = row_number()) %>%
  group_by(cluster) %>%
  summarise(
    representative = row_id[which.min(rowSums((mds_coords[row_id, ] - km$centers[cluster[1], ])^2))]
  ) %>%
  left_join(RSN_k, by = c("representative" = "row_id")) %>%
  select(cluster, Compound_Name = Compound_Name)

# Optional 2D visualization using first 2 MDS dimensions
tsne_df <- data.frame(
  X = mds_coords[,1],
  Y = mds_coords[,2],
  cluster = factor(RSN_k$cluster),
  Compound_Name = RSN_MS$Compound_Name[!is.na(RSN_MS$norm_conc)]
)

ggplot(tsne_df, aes(x = X, y = Y, color = cluster)) +
  geom_point(alpha = 0.7) +
  geom_text(data = tsne_df[tsne_df$Compound_Name %in% representatives$Compound_Name, ],
            aes(label = Compound_Name),
            color = "black", vjust = -0.5, size = 3) +
  theme_minimal() +
  labs(title = "K-means clustering on Gower distance (mixed data)")
