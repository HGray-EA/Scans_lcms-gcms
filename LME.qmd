...
compound_start <- LCGC_Lib %>%
  ...
  ungroup() %>%
  mutate(
    start_date = if_else(is.infinite(start_date), ymd("1900-01-01"), start_date)
  ) %>%
  select(Compound_Name, start_date)

# 2. Filter site of interest
site_id <- "RSN0018"
site_dates <- RSN_MS %>%
  filter(Sample_Site_ID == site_id) %>%
  distinct(Date)

# 3. Full grid: all sampled-for compounds × all dates at this site
full_grid <- compound_start %>%
  crossing(site_dates) %>%
  mutate(Sample_Site_ID = site_id)

# 4. Join actual measurements
...
# 1. Compute compound-specific minimums
compound_mins <- RSN_MS %>%
  group_by(Compound_Name) %>%
  summarise(min_conc = min(Concentration, na.rm = TRUE) / 10000, .groups = "drop")

# 2. Fill in non-detected concentrations and compute logConc per compound
RSN_MS_full <- RSN_MS_full %>%
  ...
  select(-min_conc)  # remove helper column
...
RRSN_MS$cens <- ifelse(
                      LCGC_Lib == 'Detected',
                      logConc,
                      logConc(min(RSN_MS$Concentration)/10000)
                    ) 
